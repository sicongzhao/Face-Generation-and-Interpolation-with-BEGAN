...
def conv_block(in_dim,out_dim,n_layers):
    ...
    for i  in range(n_layers):
        layers.append(nn.Conv2d(in_dim,in_dim,kernel_size=3,stride=1,padding=1))
...
def deconv_block(in_dim,out_dim,n_layers,t_conv=False):
    ...
    layers.append(nn.ELU(True))  
    for i  in range(n_layers):
        layers.append(nn.Conv2d(out_dim,out_dim,kernel_size=3,stride=1,padding=1))
        layers.append(nn.ELU(True))
    if t_conv:
...
class Encoder(nn.Module):
    ...
    def __init__(self,nc,ndf,n_layers,hidden_dim,input_dim,exp):
        '''
...
class Decoder(nn.Module):
    '''
    This is a class for dncoder module of both Generator and Discriminator.
